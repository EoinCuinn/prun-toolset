...
**The vortex drive.** `BP-CLNY-0000` is the single vortex ship in the corpus and also the largest (`V=8,532`); it carries a Vortex Reactor and Vortex Fuel Tank and has **no** standard FTL reactor. The formula does not model it at all: the vortex parts lie outside the twelve selectable fields, so its predicted volume is not a prediction and any error figure quoted against it is meaningless. It is treated as **outside the model** rather than scored — the 86/87 headline is 86 of the 86 modellable ships plus this one known exclusion. The working hypothesis — untestable on one sample — is that the vortex drive is itself **volume-triggered**, auto-fitted once a hull outgrows the top standard reactor: the largest non-vortex ship runs Hyper-Power at `V=6,242`, so any threshold lies between 6,242 and 8,532. Deliberately building an oversized hull and watching for the drive to appear is the experiment that would settle it.
